...
function unsafe_cast(::Type{BigInt}, x::BigFloat, ri::Cint)
    ...
    z = BigInt()
    ccall((:mpfr_get_z, :libmpfr), Int32, (Ref{BigInt}, Ref{BigFloat}, Int32),
          z, x, ri)
    z
end
...
function sincos_fast(v::BigFloat)
    s = BigFloat()
    c = BigFloat()
    ccall((:mpfr_sin_cos, :libmpfr), Int32, (Ref{BigFloat}, Ref{BigFloat}, Ref{BigFloat}, Int32),
          s, c, v, ROUNDING_MODE[])
    return (s, c)
end
...
function big_ln2()
    c = BigFloat()
    ccall((:mpfr_const_log2, :libmpfr), Cint, (Ref{BigFloat}, Int32),
...
function lgamma(x::BigFloat)
    z = BigFloat()
    ccall((:mpfr_lgamma,:libmpfr), Cint, (Ref{BigFloat}, Ptr{Cint}, Ref{BigFloat}, Int32), z, lgamma_signp, x, ROUNDING_MODE[])
    return z
end
...
function Base.deepcopy_internal(x::BigFloat, stackdict::ObjectIdDict)
    ...
    ccall((:mpfr_init2,:libmpfr), Void, (Ref{BigFloat}, Clong), y, N)
